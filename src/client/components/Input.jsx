...
const Input = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const response = await fetch('http://localhost:3000/unique');
...
const submitRequest = async (e) => {
  ...
  var snippets = [];
  const dbPost = async () => {
    const responseEmail =  await fetch("http://localhost:3000/bundle", { 
        method: 'POST', 
        headers: { 
...
// loop of post requests, to get body of emails, based on the the specific id (passed from the previous post request)
    ...
    for (var m =0; m<resultsArray.length; m++ ){
        const r =  await fetch("http://localhost:3000/message", { 
        method: 'POST', 
        headers: { 
          'Content-type': 'application/json'
         }, 
